...
use super::canvas_state::{CanvasState, ConnectMode, ViewMode};
use super::canvas_view;
use super::html_preview::{self, HtmlPreviewState};
...
fn build_footer_text(input_mode: InputMode, sidebar_focused: bool, selected_column: usize) -> String {
    match input_mode {
        ...
        InputMode::CanvasConnect => " Select target node [j/k] navigate [Enter] confirm [Esc] cancel ".to_string(),
        InputMode::CanvasLabel => " Type connection label [Enter] save [Esc] cancel ".to_string(),
        InputMode::HtmlPreview => " [j/k] scroll [b] open in browser [Esc] close ".to_string(),
    }
}
...
struct AppState {
    ...
    // HTML preview popup
    html_preview: Option<HtmlPreviewState>,
}
...
impl App {
    ...
    fn draw(&mut self) -> Result<()> {
        ...
        self.terminal.draw(|frame| {
            ...
            match &state.mode {
                ...
                AppMode::Project(_) => {
                    Self::draw_board(state, frame, area);
                    // Draw HTML preview popup on top if active
                    if let Some(ref preview) = state.html_preview {
                        html_preview::draw_html_preview(frame, area, preview);
                    }
                }
            }
        })?;
    ...
    fn draw_board(state: &AppState, frame: &mut Frame, area: Rect) {
        ...
        // Canvas mode: render canvas instead of board columns
        if state.view_mode == ViewMode::Canvas {
            canvas_view::draw_canvas(frame, chunks[1], &state.board.tasks, &state.canvas_state);
            // Footer for canvas mode
    ...
    fn handle_key(&mut self, key: crossterm::event::KeyEvent) -> Result<()> {
        // Handle HTML preview popup if open
        if self.state.html_preview.is_some() {
            return self.handle_html_preview_key(key);
        }

        // Handle canvas-specific input modes
        if self.state.view_mode == ViewMode::Canvas {
            match self.state.input_mode {
                InputMode::CanvasConnect => return self.handle_canvas_connect_key(key),
                InputMode::CanvasLabel => return self.handle_canvas_label_key(key),
                _ => {}
            }
        }
    ...
    fn handle_normal_key(&mut self, key: KeyCode) -> Result<()> {
        ...
        if self.state.view_mode == ViewMode::Canvas {
            return self.handle_canvas_normal_key(key);
        }
    ...
    fn switch_to_canvas(&mut self) -> Result<()> {
        self.state.view_mode = ViewMode::Canvas;
        // Auto-layout tasks that don't have positions yet
        CanvasState::auto_layout(&mut self.state.board.tasks);
        // Load connections from DB
        if let Some(ref db) = self.state.db {
            self.state.canvas_state.connections = db.get_all_connections().unwrap_or_default();
        }
        Ok(())
    }

    fn switch_to_board(&mut self) -> Result<()> {
        // Save any position changes before switching back
        self.save_canvas_positions()?;
        self.state.view_mode = ViewMode::Board;
        Ok(())
    }

    fn save_canvas_positions(&self) -> Result<()> {
        if let Some(ref db) = self.state.db {
            for task in &self.state.board.tasks {
                db.update_task_position(&task.id, task.canvas_x, task.canvas_y)?;
            }
        }
        Ok(())
    }

    fn handle_canvas_normal_key(&mut self, key: KeyCode) -> Result<()> {
        let task_count = self.state.board.tasks.len();
        match key {
            KeyCode::Char('q') => self.state.should_quit = true,
            ...
            KeyCode::Char('j') | KeyCode::Down => {
                self.state.canvas_state.select_in_direction(&self.state.board.tasks, 0.0, -1.0);
            }
            KeyCode::Char('k') | KeyCode::Up => {
                self.state.canvas_state.select_in_direction(&self.state.board.tasks, 0.0, 1.0);
            }
            KeyCode::Char('h') | KeyCode::Left => {
                self.state.canvas_state.select_in_direction(&self.state.board.tasks, -1.0, 0.0);
            }
            ...
            KeyCode::Tab => {
                // Cycle through nodes
                self.state.canvas_state.select_next(task_count);
            }
            KeyCode::BackTab => {
                self.state.canvas_state.select_prev(task_count);
            }
            KeyCode::Char('H') => {
                // Move selected node left
                self.state.canvas_state.move_selected(&mut self.state.board.tasks, -2.0, 0.0);
            }
            KeyCode::Char('J') => {
                // Move selected node down
                self.state.canvas_state.move_selected(&mut self.state.board.tasks, 0.0, -2.0);
            }
            KeyCode::Char('K') => {
            ...
            KeyCode::Char('+') | KeyCode::Char('=') => {
                self.state.canvas_state.zoom_in();
            }
            KeyCode::Char('-') => {
                self.state.canvas_state.zoom_out();
            }
            KeyCode::Char('a') => {
                // Start connection mode
                if let Some(task) = self.state.board.tasks.get(self.state.canvas_state.selected_node) {
                    self.state.canvas_state.connect_mode = ConnectMode::SelectingTarget {
                        from_task_id: task.id.clone(),
                    };
                    self.state.input_mode = InputMode::CanvasConnect;
                }
            }
            KeyCode::Char('x') => {
                // Delete connection at selected node
                self.delete_connection_at_selected()?;
            }
            KeyCode::Char('p') => {
                // Open HTML preview for selected task
                self.open_html_preview()?;
            }
            KeyCode::Char('b') => {
                // Open selected task's HTML in browser
                self.open_html_in_browser()?;
            }
            KeyCode::Enter => {
                // Open task (same as board Enter)
                if let Some(task) = self.state.board.tasks.get(self.state.canvas_state.selected_node) {
    ...
    fn handle_canvas_connect_key(&mut self, key: crossterm::event::KeyEvent) -> Result<()> {
        let task_count = self.state.board.tasks.len();
        match key.code {
            KeyCode::Char('j') | KeyCode::Down => {
                self.state.canvas_state.select_next(task_count);
            ...
            KeyCode::Enter => {
                ...
                if let ConnectMode::SelectingTarget { ref from_task_id } = self.state.canvas_state.connect_mode {
                    if let Some(target) = self.state.board.tasks.get(self.state.canvas_state.selected_node) {
                        let from_id = from_task_id.clone();
                        let to_id = target.id.clone();
                        if from_id != to_id {
                            self.state.canvas_state.connect_mode = ConnectMode::EnteringLabel {
                                from_task_id: from_id,
                                to_task_id: to_id,
                                label_buf: String::new(),
                            };
                            self.state.input_mode = InputMode::CanvasLabel;
                        }
                    }
                }
            }
            KeyCode::Esc => {
                self.state.canvas_state.connect_mode = ConnectMode::Inactive;
                self.state.input_mode = InputMode::Normal;
            }
            _ => {}
        }
    ...
    fn handle_canvas_label_key(&mut self, key: crossterm::event::KeyEvent) -> Result<()> {
        match key.code {
            KeyCode::Enter => {
                // Save connection
                if let ConnectMode::EnteringLabel { ref from_task_id, ref to_task_id, ref label_buf } = self.state.canvas_state.connect_mode {
                    let conn = crate::db::TaskConnection::new(
                        from_task_id.clone(),
                        to_task_id.clone(),
                        label_buf.clone(),
                    );
                    ...
                    self.state.canvas_state.connections.push(conn);
                }
                self.state.canvas_state.connect_mode = ConnectMode::Inactive;
                self.state.input_mode = InputMode::Normal;
            }
            ...
            KeyCode::Char(c) => {
                if let ConnectMode::EnteringLabel { ref mut label_buf, .. } = self.state.canvas_state.connect_mode {
                    label_buf.push(c);
                }
            }
            KeyCode::Backspace => {
    ...
    fn delete_connection_at_selected(&mut self) -> Result<()> {
        if let Some(task) = self.state.board.tasks.get(self.state.canvas_state.selected_node) {
            let task_id = task.id.clone();
            // Find first connection involving this task and delete it
            if let Some(conn_idx) = self.state.canvas_state.connections.iter().position(|c| {
                c.from_task_id == task_id || c.to_task_id == task_id
            }) {
                let conn_id = self.state.canvas_state.connections[conn_idx].id.clone();
                if let Some(ref db) = self.state.db {
                    db.delete_connection(&conn_id)?;
                }
                self.state.canvas_state.connections.remove(conn_idx);
            }
        }
        Ok(())
    }
    ...
    fn open_html_preview(&mut self) -> Result<()> {
        let task = if self.state.view_mode == ViewMode::Canvas {
            self.state.board.tasks.get(self.state.canvas_state.selected_node).cloned()
        } else {
            self.state.board.selected_task().cloned()
        };

        if let Some(task) = task {
            if let Some(ref html) = task.html_content {
                self.state.html_preview = Some(HtmlPreviewState::new(
                    task.title.clone(),
                    html.clone(),
                ));
                self.state.input_mode = InputMode::HtmlPreview;
            }
        }
        Ok(())
    }

    fn open_html_in_browser(&self) -> Result<()> {
        let task = if self.state.view_mode == ViewMode::Canvas {
            self.state.board.tasks.get(self.state.canvas_state.selected_node).cloned()
        } else {
            self.state.board.selected_task().cloned()
        ...
        if let Some(task) = task {
            if let Some(ref html) = task.html_content {
                let _ = html_preview::open_in_browser(html);
            }
        }
        Ok(())
    }

    fn handle_html_preview_key(&mut self, key: crossterm::event::KeyEvent) -> Result<()> {
        match key.code {
            KeyCode::Esc | KeyCode::Char('q') => {
            ...
            KeyCode::Char('j') | KeyCode::Down => {
                if let Some(ref mut preview) = self.state.html_preview {
                    preview.scroll_down();
                }
            }
            KeyCode::Char('k') | KeyCode::Up => {
            ...
            KeyCode::Char('b') => {
                // Open in browser
                if let Some(ref preview) = self.state.html_preview {
                    let _ = html_preview::open_in_browser(&preview.html_content);
                }
            }
            _ => {}
        }
