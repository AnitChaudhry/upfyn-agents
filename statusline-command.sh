...
model_full=""
if [ -n "$model_id" ]; then
  _mf=$(echo "$model_id" | sed -n 's/^claude-\([a-z]*\)-.*/\1/p')
  _mv=$(echo "$model_id" | sed -n 's/.*-\([0-9]\)-\([0-9]\)$/\1.\2/p')
  [ -z "$_mv" ] && _mv=$(echo "$model_id" | sed -n 's/.*-\([0-9]\)-[0-9]\{8\}$/\1/p')
  if [ -n "$_mf" ]; then
    _mc="$(echo "$_mf" | sed 's/^\(.\)/\U\1/')"
    [ -n "$_mv" ] && model_full="${_mc} ${_mv}" || model_full="${_mc}"
  fi
fi
[ -z "$model_full" ] && model_full="$model_display"
